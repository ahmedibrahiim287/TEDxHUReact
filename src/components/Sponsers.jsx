import React from "react";

function Sponsers() {
  return (
<div>
<div className="sponser-head">
            <h1><span className="dash-color"> - </span>An magnificence event deserves magnificence sponsors<span className="dash-color"> - </span></h1>
        </div>
        <div className="sponser-quotation">
            <p className="sponser-quote">We extend a warm thank for our Sponsors that honoured us with our event "Tangram". All thanks to thier sponsorship that were able to host our event and make it great</p>
        </div>
        <div className="sponser-grid">
            <div className="sponser-card">
                <img src="Burger-King.png" title="Burger King" alt="Burger king"/>
                <span>Burger King</span>
            </div>
             
            <div className="sponser-card">
                <img src="Chess.png" title="Chess" alt="Chess"/>
                <span>Chess</span>
            </div>
             
            <div className="sponser-card">
                <img src="Chicken-Buzz.png" title="Chicken Buzz" alt="Chicken Buzz"/>
                <span>Chicken Buzz</span>
            </div>
             
            <div className="sponser-card">
                <img className="crispy" src="Crispy-meals.png" alt="Crispy Meals" title="Crispy Meals"/>
                <span>Crispy Meals</span>
            </div>
             
            <div className="sponser-card">
                <img src="fatem.png" title="Fatem" alt="Fatem"/>
                <span>Fatem</span>
            </div>
             
            <div className="sponser-card">
                <img src="IBM.png" alt="IBM" title="IBM"/>
                <span>IBM</span>
            </div>
             
            <div className="sponser-card">
                <img src="Itqan.png" alt="Itqan" title="Itqan"/>
                <span>ITQAN</span>
            </div>
             
            <div className="sponser-card">
                <img src="Panasonic.png" alt="Panasonic" title="Panasonic"/>
                <span>Panasonic</span>
            </div>
            <div className="sponser-card">
                <img src="qahwargy.png" alt="qahwargy" title="qahwargy"/>
                <span>qahwargy</span>
            </div>
            <div className="sponser-card">
                <img src="Ravin.png" alt="Ravin" title="Ravin"/>
                <span>Ravin</span>
            </div>
            <div className="sponser-card">
                <img src="Silla-Sweets.png" alt="Silla Sweets" title="Silla Sweets"/>
                <span>Silla Sweets</span>
            </div>
            <div className="sponser-card">
                <img src="Sira.png" alt="Sira" title="Sira"/>
                <span>Sira</span>
            </div>
            <div className="sponser-card">
                <img src="Snow-lite.png" alt="Snow lite" title="Snow lite"/>
                <span>Snow lite</span>
            </div>
            <div className="sponser-card">
                <img src="TATAS.png" alt="TATAS" title="TATAS"/>
                <span>TATAS</span>
            </div>
            <div className="sponser-card">
                <img src="TE-Data.png" alt="TE-Data" title="TE-Data"/>
                <span>TE-Data</span>
            </div>
            <div className="sponser-card">
                <img src="TeensStuff.png" alt="TeensStuff" title="TeensStuff"/>
                <span>TeensStuff</span>
            </div>
            <div className="sponser-card">
                <img src="Tut_s-Socks.png" alt="Tuts Socks" title="Tuts Socks"/>
                <span>Tuts Socks</span>
            </div>
            <div className="sponser-card">
                <img src="TicketsMarch.png" alt="Tickets March" title="Tickets March"/>
                <span>Tickets March</span>
            </div>
        </div>
        {/* 
           __   
        .__(.)< (Buck Buck)
        (____)
           __   
        .__(.)< (Buck Buck)
        (____)
           __   
        .__(.)< (Buck Buck)
        (____)
        */}
</div>
  );
}

export default Sponsers;
